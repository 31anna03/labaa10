...
#pragma endregion
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		int size = Convert::ToInt32(textBox1->Text); // Отримуємо розмір черги
		if (myQueue1.Count < size) {
			...
			if (valueInput == "") {
				return; // Користувач не ввів значення
			}

			int inputValue = Convert::ToInt32(valueInput);
			myQueue1.Enqueue(inputValue); // Додаємо елемент до черги
			listBox1->Items->Add(inputValue.ToString()); // Оновлюємо список
